...
| Artikelzeile | Tabelle `Blog & Newsletter` | `tblzTKBLsewsvoCis` |
...
Pro genutztem Bild in `Blog-Bilder`: `Nutzungszähler Blog` um 1 erhöhen, Webflow-URL eintragen, Verknüpfung zur `Blog & Newsletter`-Zeile setzen. Bei der genutzten Vorlage in `Grafik-Vorlagen` ebenfalls `Nutzungszähler` um 1 erhöhen. Beides passiert **nur beim tatsächlichen Publish**, nie beim Vorschlagen – so zählt nur, was wirklich live ging. Offener Punkt (Stand 08/2026): Diese Zähler erhöht das Make-Szenario „Content-Publishing" (ID 9521520) noch nicht – bis das nachgebaut ist, setzt Ali sie beim Publish manuell.
