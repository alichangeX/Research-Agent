...
description: >
  ...
  LinkedIn-Themen als Nächstes in die Content-Pipeline (Airtable) sollen. Deckt
  den kompletten Planungsprozess ab: Themenfindung, Priorisierung, Zuordnung zu Kanälen
  (Blog/Newsletter/LinkedIn), Buyer-Stage-Mapping und Übergabe an die
...
**Einbettung in die Content-Pipeline:** Themen landen als Zeilen in Airtable (Base `appb7eOfe2Au3Lp40`), und zwar je Kanal in einer eigenen Tabelle: Blog- und Newsletter-Themen in "Blog & Newsletter", LinkedIn-Themen in "LinkedIn-Content". Die Kanalzuordnung ist damit die Tabellenwahl, Kanal-Checkboxen gibt es nicht. Jede Themenidee, die dieser Skill ausgibt, muss so aufbereitet sein, dass sie direkt in diese Struktur passt: Status "Neu recherchiert (P1)", Themenpillar (Feld "Cluster/Pillar") und die Bewertungsachsen. Läuft dieser Skill innerhalb der monatlichen Recherche-Routine, ersetzt er die manuelle Themenfindung unten; das Freigabe-Gate durch Christoph (Auswahl der finalen Themen) bleibt in jedem Fall manuell.
...
Vor jeder größeren Planungsrunde prüfen, was in den Tabellen "Blog & Newsletter" und "LinkedIn-Content" bereits im Status "Neu recherchiert (P1)" oder "Ausgewählt" liegt, um Doppelplanung zu vermeiden.

---

## Leitquellen-Regel (roter Faden)

Jedes Thema bekommt genau **eine Leitquelle** - die eine Studie, den einen Artikel oder das eine interne Projektbeispiel, das die Kernaussage trägt. Alle weiteren Quellen sind ausschließlich unterstützender Kontext für diese eine Kernaussage, nie eine zweite, gleichrangige Aussage.

- Zwei Signale zum selben Themenfeld, aber mit unterschiedlicher Kernaussage aus unterschiedlichen Quellen → **zwei getrennte Themen-Kandidaten** anlegen, nicht zu einem verschmelzen. Das gilt schon in der Clusterphase, nicht erst bei der Texterstellung.
- Bei der Ausgabe (siehe Output-Format unten) immer benennen: welche Quelle ist die Leitquelle, welche (falls vorhanden) sind nur Stützbelege für dieselbe Aussage.
- Grund: Wenn zwei Studien mit je eigener Kernaussage in einem Blogartikel/Post gebündelt werden, geht der rote Faden verloren und wichtige Kernaussagen der Einzelquellen fallen unter den Tisch. Ein Thema, eine Leitquelle, eine Kernaussage - das gilt für Blog, Newsletter und LinkedIn gleichermaßen.
...
Damit die Themen direkt in die passende Airtable-Tabelle ("Blog & Newsletter" bzw. "LinkedIn-Content") übernommen werden können, immer liefern:
...
- Buyer-Stage
- Kanal-Eignung und damit Zieltabelle: Blog und/oder Newsletter → "Blog & Newsletter"; LinkedIn (Christoph) → "LinkedIn-Content"
...
- Leitquelle (die eine Studie/der eine Artikel/das eine Projektbeispiel, das die Kernaussage trägt) - weitere Quellen nur als Stützbeleg kennzeichnen, siehe Leitquellen-Regel oben
...
- [ ] Jedes Thema hat genau eine benannte Leitquelle mit einer Kernaussage - keine zwei gleichrangigen Studien/Artikel in einem Thema gebündelt (siehe Leitquellen-Regel)
...
- [ ] Geprüft, ob Thema nicht bereits in "Blog & Newsletter", "LinkedIn-Content" oder in der Archivtabelle existiert
- [ ] Output ist direkt airtable-fähig (Zieltabelle, Status, Cluster/Pillar, Bewertungsachsen)
